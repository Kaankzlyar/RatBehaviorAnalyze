function varargout = Kutu_v1(varargin)
%KUTU_V1 MATLAB code file for Kutu_v1.fig
%      KUTU_V1, by itself, creates a new KUTU_V1 or raises the existing
%      singleton*.
%
%      H = KUTU_V1 returns the handle to a new KUTU_V1 or the handle to
%      the existing singleton*.
%
%      KUTU_V1('Property','Value',...) creates a new KUTU_V1 using the
%      given property value pairs. Unrecognized properties are passed via
%      varargin to Kutu_v1_OpeningFcn.  This calling syntax produces a
%      warning when there is an existing singleton*.
%
%      KUTU_V1('CALLBACK') and KUTU_V1('CALLBACK',hObject,...) call the
%      local function named CALLBACK in KUTU_V1.M with the given input
%      arguments.
%
...
% Edit the above text to modify the response to help Kutu_v1

% Last Modified by GUIDE v2.5 12-Feb-2019 15:34:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Kutu_v1_OpeningFcn, ...
                   'gui_OutputFcn',  @Kutu_v1_OutputFcn, ...
                   'gui_LayoutFcn',  [], ...
                   'gui_Callback',   []);
...
% --- Executes just before Kutu_v1 is made visible.
function Kutu_v1_OpeningFcn(hObject, eventdata, handles, varargin)
...
% Choose default command line output for Kutu_v1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Kutu_v1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Kutu_v1_OutputFcn(hObject, eventdata, handles)
...
[v_file,v_path] = uigetfile( ...
    {'*.avi;*.mp4;*.mov;',...
    'Video Dosyas� (*.avi;*.mp4;*.mov;)'},...
    'Deney Kayd�n�n Tutuldu�u Video Dosyas�n� Se�iniz.');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
P = [handles.pnt1;handles.pnt2;handles.pnt3;handles.pnt4;handles.pnt1];
P2 = [handles.pnt5;handles.pnt6;handles.pnt7;handles.pnt8;handles.pnt5];
...
axes(handles.axes3);
imshow(mFrame);
hold on, plot(handles.X,handles.Y,'LineWidth',3,'Color','r');
plot(handles.X2,handles.Y2,'LineWidth',3,'Color','g'); hold off,
axis normal;
...
%     Rat((frameHm>0)&(frameHm<0.2)) = 1; %TMaze 1-3-5
%     Rat((frameHm>0)&(frameHm<0.13)) = 1; %TMaze 7 Box 1-3-5-7
    Rat((frameHm>0.12)&(frameHm<0.2)) = 1; %Box Final 7
    ...
    imshow(Shw,[]);drawnow;
    OutCenter = (bmn(ii)<handles.pnt5(1))|(bmx(ii)>handles.pnt6(1))...
        |(amn(ii)<handles.pnt5(2))|(amx(ii)>handles.pnt8(2));
    if OutCenter
        Sides(ii) = 1;
        Center(ii) = 0;
    else
    ...
    title(sprintf('Time %.2f, Sides %.0f, Center %.0f',t(ii), Sides(ii), Center(ii)));
