...
P = [pnt1;pnt2;pnt3;pnt4;pnt5;...
    pnt6;pnt7;pnt8;pnt9;pnt10;...
    pnt11;pnt12;pnt1];
...
mxc = ((xc_filtered-min(X))/(max(X)-min(X)))*1;
myc = ((yc_filtered-min(Y))/(max(Y)-min(Y)))*1;
...
fprintf('X Yönünde Hareketin Varyansı = %.4f\n',((mxc-mean(mxc))*(mxc-mean(mxc))')/numel(mxc))
fprintf('Y Yönünde Hareketin Varyansı = %.4f\n',((myc-mean(myc))*(myc-mean(myc))')/numel(myc))
fprintf('Hareketin Varyansı = %.4f\n',((mmc-mean(mmc))*(mmc-mean(mmc))')/numel(mmc))
Res = [max(vx),max(vy),...
    min(mxc),max(mxc),...
    min(myc),max(myc),...
    ((mxc-mean(mxc))*(mxc-mean(mxc))')/numel(mxc),...
